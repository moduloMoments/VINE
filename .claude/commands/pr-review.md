...
**validate the `vine-reviewer` role recipe** (the agent definition in `plugins/vine/agents/vine-reviewer.md`) by
...
The reviewer role recipe lives in the `vine-reviewer` agent definition (`plugins/vine/agents/vine-reviewer.md`) —
...
Confirm `plugins/vine/agents/vine-reviewer.md` exists (it surfaces as the `vine-reviewer` agent type). If it's
...
- **Missing context log** — empty (recipe + artifacts sufficient) or, if not, list each gap as a
  candidate fix to `plugins/vine/agents/vine-reviewer.md`, `references/STATE.md`, or the artifact templates.
