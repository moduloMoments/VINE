...
name: vine-evolve
description: >
  VINE Phase 4: Evolve — Triple evolution of product, agent, and user after feature implementation.
  This phase verifies the implementation against acceptance criteria from SPEC.md, then drives three
  evolutions: product quality (verification, PR prep, follow-up work), agent capability (CLAUDE.md
  updates, new skill suggestions, workflow improvements), and user growth (knowledge gained, areas
  to explore, learning captured). Produces EVOLUTION.md and a handoff package.
  Use this skill after vine:navigate when implementation is complete and ready for review, or when
  the engineer says "let's verify this", "are we done", "prep for PR", "retro on this feature",
  "what did we learn", "evolve", "wrap up."
  Trigger on: "vine:evolve", "verify the implementation", "check acceptance criteria", "prep for PR",
  "retro", "what did we learn", "wrap this up", "feature retro", "vine evolve", "are we done",
  "let's review what we built", "evolve the product", "capture learnings".
---
