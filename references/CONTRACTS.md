...
- **Route**: [interactive | headless] — `mechanism: [how it ran, or n/a]` <!-- optional -->
...
- **Controlled Route vocabulary + `mechanism:` token.** `**Route**` uses exactly `interactive | headless` followed by a labeled `mechanism:` token naming *how* it ran (or `n/a`). The vocabulary is closed so the field stays machine-comparable across journals.
...
- **Mechanism-divergence correction (the #90 wrong-extraction gap).** If the run's mechanism diverges mid-slice from what the `Route` field recorded (e.g. a `vine-coder` run recorded `headless — mechanism: vine-coder agent` is escalated on a `human-required` decision and a human finishes the slice interactively), **correct the `Route`/`mechanism:` token to what actually ran** and note the divergence in `**Deviations from spec**`. A prose note alone is insufficient: the field itself must be made true, because mechanical extraction reads the field, not the prose around it.
...
### The journal-check hook and `.vine/scripts/`

VINE's one mechanical guarantee — journal-before-commit — is a native hook that **ships with the plugin**, default-on: `plugins/vine/hooks/journal-check.sh`, wired via `plugins/vine/hooks/hooks.json` (PreToolUse on Bash, resolved under `${CLAUDE_PLUGIN_ROOT}`). Installing VINE turns it on; there is no scaffold step. It is POSIX sh, reads the `ACTIVE` sentinel at `.vine/ACTIVE` (per-working-tree, no git resolution), treats its feature path as an opaque repo-relative string, and **fails open**: no sentinel, missing tooling, or ambiguity exits 0 — enforcement degrades, sessions never break.

| Hook | Hook event | Behavior |
|------|------------|----------|
...
`.vine/scripts/` is no longer a user scaffold — it is the VINE framework repo's home for **contributor-only** enforcement tooling, none of which is in the plugin payload: `trellis-check.sh` (the mechanical check engine that runs trellis's skill and cross-reference anchor checks and writes the `.vine/.trellis-ok` stamp), `trellis-gate.sh` (the skill-commit gate that reads that stamp), `main-guard.sh` (blocks commits on `main`), and `run-tests.sh` (the CI test harness). These are wired by this repo's own `.claude/settings.json` and `.github/workflows/`, never distributed.
...
Unlike the overlays, the README configures nothing — overlays steer VINE's behavior, the README documents it. It points back to this file (CONTRACTS.md) for the authoritative artifact structure rather than duplicating it, so it can't drift from the contract.
