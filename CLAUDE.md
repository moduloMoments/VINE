...
- When adding to a skill, prefer an unnumbered `###` section over renumbering existing steps — renumbering ripples through every `step N` cross-reference (in the skill and in `references/STATE.md`), the exact drift `/trellis` Check 10 catches. An unnumbered section also signals "runs once / out of the numbered flow" (e.g. navigate's Completion Gate Check)
