...
name: vine-navigate
description: >
  VINE Phase 3: Navigate — Guided implementation with two-way learning and human review gates.
  This phase takes the SPEC.md from vine:inquire and implements the feature one work slice at a
  time. The engineer and Claude navigate the implementation together — the engineer steers direction
  and provides course corrections while learning from Claude's approach, and Claude learns from
  the engineer's domain expertise. No auto-commits. Changes are staged and surfaced for review.
  Every decision is documented in a NAVIGATION.md log.
  Use this skill after vine:inquire when you have a SPEC.md and are ready to implement, or when
  the engineer says "let's start building", "implement this", "code this up", "navigate the feature."
  Trigger on: "vine:navigate", "let's start building", "implement this", "start coding",
  "build the feature", "navigate", "work on slice", "next slice", "vine navigate",
  "let's write some code", "implement slice", "continue implementation".
---
